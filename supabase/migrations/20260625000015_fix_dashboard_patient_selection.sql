-- supabase/migrations/20260625000015_fix_dashboard_patient_selection.sql
...
begin
  ...
  -- Fix: Find the explicit patient in the group
  ...
  -- Fallback if no strict patient role is found (e.g. legacy groups)
  ...
  select jsonb_build_object(
    ...
    'today_summary', (
      select jsonb_build_object(
        ...
        'meds_taken', (
            select count(*) from public.medicine_logs ml where ml.user_id = v_patient_id and date(ml.scheduled_time at time zone 'utc') = current_date and ml.status = 'taken'
        ),
        'tasks_due_today', (
        ...
        'appointments_today', (
            select count(*) from public.appointments a where a.user_id = v_patient_id and date(a.appointment_date at time zone 'utc') = current_date
        )
      )
    ),
    'top_tasks', (
    ...
    'upcoming_events', (
      ...
      from (
        ...
        where group_id = v_group_id and start_at >= now()
